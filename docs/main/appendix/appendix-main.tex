...
\subsection*{A.1.1 Included in this bachelor's thesis:}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item Lab 1: PEBCAK Corp — Recon & Enumeration
        \item Lab 2: SYNAPSE Intelligence Portal — Web Vulns
        \item Lab 3: HELIX Systems — Incident Response
        \item Lab 4: CipherStrike — Crypto+Stego CTF
    \end{itemize}
    \item \textbf{22 EVE-NG nodes across 4 labs}, built from 5 base image types:
    \begin{itemize}
        \item 4x pfSense CE (perimeter firewalls, one per lab)
        \item 4x VyOS (internal routers, one per lab)
        \item 5x Ubuntu Server 24.04 (target services: web, database, crypto/stego)
        \item 2x Ubuntu Desktop 24.04 (internal workstation PC1; monitoring Defender)
        \item 4x Parrot OS (attacker/analyst nodes, one per lab)
    \end{itemize}
    \item \textbf{Documentation and build workflow scripts}:
    \begin{itemize}
        \item Makefile with targets for LaTeX compilation (Vol~I, Vol~II, labs)
        \item Shell scripts for PDF generation and GitHub Pages deployment
        \item Python provisioning scripts for EVE-NG node configuration
        \item Git-based version control for all topology and documentation files
    \end{itemize}
    ...
    \item \textbf{Student learning guides} (15-30 pages per lab):
    \begin{itemize}
