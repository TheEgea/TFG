...
\midrule
1 & A03:2021 & Stored XSS in report field $ightarrow$ session cookie theft & -- \\
2 & A07:2021 & Cookie injection $ightarrow$ admin panel access & -- \\
3 & A03:2021 & SQL injection in search $ightarrow$ credential dump & FLAG\{synapse\_sqli\_creds\_dumped\} \\
...
which caused pfSense to silently discard packets arriving from the simulated
...
when entering the initial credentials. Corrected by explicitly selecting the
...
reachable on any configured interface. The issue was that the intended next-hop was
on a subnet not yet assigned to any VyOS interface at the time the route was applied.
Resolved by defining the interface address before the static route, and by using
