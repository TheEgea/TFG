...
Topology: multi-tier segmented network with perimeter firewall, internal router, target server, and internal workstation.
...
Tools: standard Unix log analysis utilities (\texttt{grep}, \texttt{awk},
\texttt{find}, \texttt{journalctl}, \texttt{cat}).
Topology: multi-segment network with pre-staged attack artefacts.
Validation: successful timeline reconstruction and IOC identification.
...
\item \textbf{Lab 4: CipherStrike --- Cryptography and Steganography CTF} (16h research + 30h dev) ---
Scenario: Capture-the-Flag challenges covering cryptographic attacks and steganography
analysis across three servers (A: crypto, B: stego, C: combined gate challenges).
Tools: Python cryptanalysis scripts (\texttt{pycryptodome}, \texttt{gmpy2}), steganography tools (\texttt{steghide}, \texttt{exiftool}, PIL), hash cracking, encoding/decoding utilities.
Validation: all 14 flags captured (A1--A5, B1--B5, C1--C4).
...
\item LaTeX (XeLaTeX + OpenDyslexic) for formal academic documentation (Volume~I)
