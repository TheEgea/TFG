...
individual lab is then described in its own section.
...
standard EVE-NG path \texttt{/opt/unetlab/addons/qemu/}. Each system was placed in
a dedicated directory with a normalised name following the pattern
\texttt{<type>-<version>} (e.g., \texttt{pfsense-2.7.2},
\texttt{vyos-1.x-rolling-amd64}, \texttt{linux-ubuntu-server-24.04}).

For GNU/Linux systems, the \texttt{linux-} prefix was applied consistently
(e.g., \texttt{linux-ubuntu-desktop-24.04}, \texttt{linux-parrot-rolling}), since
...
image named \texttt{cdrom.iso} and a \texttt{virtioa.qcow2} disk image sized
...
creation under \texttt{/opt/unetlab/addons/qemu/}, renaming the ISO to
\texttt{cdrom.iso}, creation of the \texttt{virtioa.qcow2} disk at a size inferred
...
Lab 2: Web Application Vulnerabilities & OWASP Top~10 exploitation~\cite{owasp2021}              & Implemented \\
Lab 3: Network Analysis                & Traffic capture, lateral movement       & Pending \\
