...
\item \textbf{RF1.1}: Four topologies, each targeting a specific pentesting phase:
  ...
  \item Lab 4: Cryptography and steganography CTF (CipherStrike --- applied crypto and stego)
  \end{itemize}
\end{itemize}
...
\item \textbf{RF2.4}: Lab 4 --- Cryptography and Steganography CTF (CipherStrike):
      students act as incident response analysts investigating a breach at NovaCorp.
      ServerA hosts five classical and modern cryptography challenges (ROT13, AES-ECB oracle,
      Vigen\`{e}re/Kasiski, XOR crib-dragging, RSA $e=3$); ServerB hosts five steganography
      challenges (EXIF metadata, PNG chunk appending, LSB red/alpha channels, steghide);
      ServerC hosts four advanced combined challenges (multilayer stego, onion encoding,
      SHA-1 length extension, ECDSA nonce reuse). Access to ServerC is gated on solving
      key challenges from the first two modules.
\end{itemize}
...
\item \textbf{RF3.2}: Detailed assessment rubric per lab (100~pt scale with explicit
      criteria), included in the teacher resolution guide.
\end{itemize}

\textbf{Success Criteria}: 4 student lab guides and 4 teacher resolution guides (PDF),
each including a rubric.
...
\begin{itemize}
\item \textbf{TR1.1}: EVE-NG 5.0+ Community Edition, deployed as a
      VM on Proxmox VE; web interface via HTTPS; RESTful API for automation.

\item \textbf{TR1.2}: Host specifications: 2$\times$ Intel Xeon E5-2680v4 (56 logical cores),
...
\item \textbf{TR2.1}: Attacking platform: Parrot OS Security 6.x~\cite{parrot_os} (primary).
      Selected over Kali Linux for its lower memory and disk footprint in concurrent EVE-NG
...
Additional sub-requirements (RF1.2--RF1.4, RF2.5, RF3.3--RF3.4, TR1.3, TR1.4, TR2.4, TR3.3) extending those defined above, together with the full NFR attribute descriptions, are provided in Appendix~H.
