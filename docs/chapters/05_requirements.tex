...
This chapter specifies the complete requirements for the EVE-NG-based~\cite{eve_ng} cybersecurity teaching package, encompassing functional requirements (RF) that define what the system must do, technological requirements (TR) that specify the technical environment, and non-functional requirements (NFR) that address quality attributes and compliance standards.
...
\textbf{Success Criteria}: See technical KPIs in Chapter~3 (Table~\ref{tab:kpi_technical}).

\subsection{RF2: Implement Vulnerabilities and Attack Scenarios}

\textbf{Description}: Implement realistic vulnerabilities and attack scenarios aligned with industry frameworks (PTES\cite{ptes}, OWASP\cite{owasp_testing_guide}, NIST\cite{nist_framework}, CEH\cite{ceh_framework}).
...
\item Active scanning tools (Nmap\cite{nmap}, banner grabbing)
...
\item Network traffic analysis with Wireshark\cite{wireshark}
...
\item Linux privilege escalation: SUID/SGID binaries, sudo misconfigurations, writable cron jobs, and capabilities
...
\textbf{Success Criteria}: Each lab must include a minimum of 3--4 exploitable vulnerability scenarios, all resolvable and documented with remediation procedures.

\subsection{RF3: Create Structured Assessment Materials}

\textbf{Description}: Develop comprehensive assessment rubrics, learning objectives, and evaluation materials aligned with GEISI curriculum~\cite{geisi_curriculum}.
...
\begin{itemize}
\item Detailed assessment rubric per lab with quantifiable criteria
\item Point allocation per criterion (clear grading scale)
\item Included in the instructor resolution guide
\end{itemize}
...
\textbf{Success Criteria}: 4 student lab guides (enunciado PDF) and 4 instructor resolution guides (resolució PDF), each including an assessment rubric. All lab features documented.
...
\item Deployment: Virtual machine on Proxmox VE host or bare-metal installation
...
\begin{itemize}
\item Primary: Parrot OS Security 6.x~\cite{parrot_os}
\item Alternative: Kali Linux~\cite{kali_linux}
...
\end{itemize}
\textbf{Rationale}: Parrot OS Security was selected over Kali Linux as the primary
attacking platform for three reasons. First, its base system is significantly lighter
in terms of disk and memory footprint, which is relevant when running multiple VMs
concurrently inside EVE-NG on constrained lab hardware. Second, the Parrot image
integrates cleanly into EVE-NG without requiring manual QEMU parameter adjustments.
Third, Parrot's desktop environment and tool organisation are more approachable for
students who are not yet familiar with dedicated cybersecurity distributions, reducing
the cognitive overhead of the attacker node itself so that students can focus on the
lab objectives.

\item \textbf{TR2.2}: Core Security Tools (installed on Parrot OS Security)
\begin{itemize}
\item \textbf{Reconnaissance}: Nmap, Shodan CLI, Amass, Sublist3r
\item \textbf{Scanning}: Nessus (optional), OpenVAS, Nikto
\item \textbf{Exploitation}: Metasploit Framework 6.x~\cite{metasploit}, ExploitDB, Searchsploit
\item \textbf{Web Testing}: Burp Suite Community/Professional~\cite{burp_suite}, OWASP ZAP~\cite{owasp_zap}
\item \textbf{Network Analysis}: Wireshark\cite{wireshark}, tcpdump, mitmproxy
%\item \textbf{Post-Exploitation}: Mimikatz, Rubeus, BloodHound  % Windows AD tools -- reserved for Lab 4 if Windows targets are added
\item \textbf{Programming}: Python 3.10+, Bash scripting
\end{itemize}

\item \textbf{TR2.3}: Target System Images
\begin{itemize}
\item Ubuntu Server 24.04 LTS (Linux target servers)
\item Ubuntu Desktop 24.04 LTS (internal workstation)
\item pfSense CE (perimeter firewall and NAT)
\item VyOS (routing and inter-VLAN switching)
\item Vulnerable web application: SYNAPSE (custom, Lab~2)
\end{itemize}
...
\begin{itemize}
\item At least one utility script per lab for a recurring management task
\item ISO upload automation (\texttt{Iso\_uploader.py}): SCP-based image onboarding
\item Bridge configuration and connectivity check scripts
\item All scripts documented with usage instructions in the repository
\end{itemize}

\item \textbf{TR3.3}: Integration Points
\begin{itemize}
\item EVE-NG API integration for programmatic lab management
%\item Webhook support for event-driven automation  % not implemented
\item Docker for internal lab application deployment (e.g., SYNAPSE web app in Lab~2)
...
\begin{itemize}
\item Current infrastructure supports 3--4 concurrent lab instances on the project HomeLab server
\item Architecture is horizontally scalable: additional EVE-NG nodes can be added on institutional infrastructure
\item EVE-NG Professional Edition supports larger concurrent deployments for full classroom use
\end{itemize}
...
\item \textbf{NFR2.1}: NIST Cybersecurity Framework Alignment~\cite{nist_framework}
\begin{itemize}
...
\item \textbf{NFR2.2}: CIS Critical Controls Coverage~\cite{cis_controls}
\begin{itemize}
...
\item \textbf{NFR2.3}: OWASP Standards Integration~\cite{owasp2021,owasp_testing_guide}
\begin{itemize}
...
\item \textbf{NFR2.4}: Penetration Testing Standards~\cite{ptes,ceh_framework}
\begin{itemize}
...
%\item Video tutorials for complex procedures (2-3 minutes each)  % not produced
%\item FAQ documentation (minimum 50 entries)  % not produced
